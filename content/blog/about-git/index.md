...
1. **_master_** 브랜치의 _text.txt_ 에 내용을 추가하고 커밋한다.
...
<br>
<!-- ![merge7.5](./content-pic/mg7.5.png)
![merge7.6](./content-pic/mg7.6.png) -->

2. **_dev1_**과 **_dev2_**브랜치를 만들고 **_dev1_**브랜치로 이동한다.
...
![merge8](./content-pic/mg8.png)

3. **_dev1_**브랜치의 _text.txt_ 에 내용을 추가하고 커밋하면 아래와 같은 상태이다.
...
```
$ git add .
$ git commit -m "dev1 message"
~
```

<!--
![merge8.1](./content-pic/mg8.1.png)

![merge8.2](./content-pic/mg8.2.png) -->

![merge9](./content-pic/mg9.png)

4. 이번에는 **_dev2_**브랜치로 이동해서 내용을 추가한뒤 변경 사항을 커밋한다.
...
```
$ git add .
$ git commit -m "dev2 message"
~
```

<!--
![merge10](./content-pic/mg10.png)
![merge11](./content-pic/mg11.png) -->

![merge12](./content-pic/mg12.png)

5. **_dev1_**과 **_dev2_**에서 변경한 부분을 모두 **_master_** 브랜치에 통합해보자. 먼저 **_master_** 브랜치로 이동한 다음 **_dev1_**브랜치를 병합한다.

```
$ git checkout master
Switched to branch 'master'
$ git merge dev1
Updating 8a7c63e..b4b828c
Fast-forward
 text.txt | 1 +
...
여기서는 같은 파일의 같은 부분을 수정했기 때문에 충돌이 발생한다. 파일을 열어보면 다음과 같다.
...
`fetch`란 **'가져오다, 불러오다'**라는 뜻이다. 따라서 git fetch는 원격 저장소의 정보를 가져오는 기능이 있다. `pull` 명령이 원격 저장소의 커밋을 가져와서 무조건 지역 저장소와 자동으로 병합 작업을 한다면, `fetch` 명령은 원격 저장소의 내용을 확인만 하고 로컬 데이터와 병합은 하지 않을 수 있다.
